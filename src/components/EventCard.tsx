  ...
  return (
    <article className="rounded border border-slate-200 bg-white p-5 shadow-sm transition duration-300 hover:-translate-y-1 hover:border-copper/60 hover:shadow-xl">
      <div className="flex flex-wrap items-center gap-2">
        <span className="rounded bg-copper/10 px-2.5 py-1 text-xs font-semibold text-copper">
      ...
      <div className="mt-4 grid gap-2 text-sm text-slate-600">
        <span className="flex items-start gap-2">
          <Clock size={16} className="mt-0.5 shrink-0 text-tealstone" />
      ...
      {!compact && (
        <p className="mt-4 text-sm leading-6 text-slate-600">{textOf(event.abstract, i18n.language)}</p>
      )}
